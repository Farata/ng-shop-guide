...
----
+
By default Angular imposes strict security rules and doesn't allow using arbitrary strings as values for image source URLs. `DomSanitizer` allows us to bypass these rules for values we know are safe.
